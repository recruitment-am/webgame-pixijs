...
export default class Logger {
  ...
  debug(message: any | string, ...args: any[]) {
    if (!this.logDebug) return;
    if (typeof message === 'string') message = `[dbg] ${message}`;
    else if (message && typeof message.message === 'string') message.message = `[dbg] ${message.message}`;
  ...
  log(message: any | string | number, ...args: any[]) {
  ...
  error(message: object | string, ...args: any[]) {
    ...
    // TODO: POST the error on remote for further investigation
  }
